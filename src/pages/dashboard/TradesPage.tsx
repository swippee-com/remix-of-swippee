...
function exportCSV(trades: any[]) {
  ...
  const rows = trades.map((t) => [
    t.side, t.asset, t.gross_amount, t.net_amount, t.fee_amount,
    t.quoted_rate, t.fiat_currency, t.status, format(new Date(t.created_at), "yyyy-MM-dd"),
  ]);
...
async function exportPDF(trades: any[]) {
  ...
  (doc as any).autoTable({
    ...
    body: trades.map((t) => [
      t.side, t.asset, `${t.gross_amount} ${t.asset}`,
      `${Number(t.quoted_rate).toLocaleString()} ${t.fiat_currency}`,
      t.fee_amount, t.status.replace(/_/g, " "),
        ...
        {trades.length > 0 && (
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              <DropdownMenuItem onClick={() => exportCSV(filteredTrades)}>
                <FileSpreadsheet className="mr-2 h-4 w-4" /> Export CSV
              </DropdownMenuItem>
              <DropdownMenuItem onClick={() => exportPDF(filteredTrades)}>
